...
//Minimum number of nodes returned before filtering in a getaddr call
#define ADDRMAN_GETADDR_MIN_FILTER 750
...
protected:
    ...
    void GetAddr_(std::vector<CAddress> &vAddr, enum Network net = NET_UNROUTABLE);
...
public:
    ...
    std::vector<CAddress> GetAddr(enum Network net = NET_UNROUTABLE)
    {
        Check();
